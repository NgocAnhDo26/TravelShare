 ...
 */
export const likeTarget = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (!Types.ObjectId.isValid(targetId) || !['TravelPlan', 'Post'].includes(onModel)) {
 ...
 */
export const unlikeTarget = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { onModel, targetId } = req.params;
    const userId = req.user as string;
 ...
 */
export const getUsersWhoLiked = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { onModel, targetId } = req.params;
 ...
 */
export const getLikedTargetsByUser = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const userId = req.user as string;
    ...
    if (model !== 'TravelPlan') {
      res.status(400).json({ message: 'Only TravelPlan likes are supported in this endpoint.' });
      return;
    }
