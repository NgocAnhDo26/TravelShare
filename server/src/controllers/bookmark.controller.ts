...
const BookmarkController = {
  toggleBookmark: asyncHandler(async (req: Request, res: Response) => {
    ...
    const result = await BookmarkService.toggleBookmark(userId, targetId, onModel);
  ...
  getMyBookmarks: asyncHandler(async (req: Request, res: Response) => {
    ...
    const result = await BookmarkService.getBookmarksForUser(userId, filter as 'all' | 'plans' | 'posts', page, limit);
    res.status(200).json(result);
  }),
