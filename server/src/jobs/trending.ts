...
export async function updateTrendingScores() {
  ...
  try {
    ...
    for (const plan of recentPlans) {
      ...
      const hoursSinceCreation = (now - plan.createdAt!.getTime()) / (1000 * 60 * 60);

      const score = engagementPoints / Math.pow(hoursSinceCreation + 2, GRAVITY);
      ...
      if (Math.abs((plan.trendingScore || 0) - score) > 0.0001) {
         await TravelPlan.findByIdAndUpdate(plan._id, { trendingScore: score });
         updatedCount++;
      }
    }

    console.log(`Trending score update job finished. Updated ${updatedCount} plans.`);
  } catch (error) {
