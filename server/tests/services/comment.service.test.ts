...
describe('CommentService', () => {
  ...
  describe('getCommentsForTarget', () => {
    it('should return comments for a target with pagination', async () => {
      ...
      const result = await CommentService.getCommentsForTarget(targetId, 'TravelPlan', 1, 5);

      expect(mockComment.find).toHaveBeenCalledWith({ targetId, onModel: 'TravelPlan', parentId: null });
  ...
  describe('addComment', () => {
    it('should add a new top-level comment and update target comment count', async () => {
      ...
      mockTravelPlan.findById.mockReturnValue({ session: vi.fn().mockResolvedValue({ _id: targetId }) } as any);
      mockTravelPlan.updateOne.mockResolvedValue({} as any);
      
      vi.spyOn(CommentService, 'getCommentById').mockResolvedValue({ ...mockCommentLean, _id: newCommentId } as any);

      const result = await CommentService.addComment(userId, targetId, 'TravelPlan', { content: 'New comment' });
      ...
      expect(mockTravelPlan.updateOne).toHaveBeenCalledWith({ _id: targetId }, { $inc: { commentsCount: 1 } }, { session: mockSession });
    ...
    it('should throw an error if content and imageUrl are missing', async () => {
        await expect(CommentService.addComment(userId, targetId, 'TravelPlan', {}))
            .rejects.toThrow('Comment must have content or an image.');
    });

    it('should rollback transaction on error', async () => {
        mockTravelPlan.findById.mockReturnValue({ session: vi.fn().mockRejectedValue(new Error('DB Error')) } as any);

        await expect(CommentService.addComment(userId, targetId, 'TravelPlan', { content: 'test' }))
  ...
  describe('updateComment', () => {
    it('should update a comment successfully', async () => {
        ...
        const mockUpdatedComment = { ...mockCommentLean, content: updatedContent };
        ...
        const result = await CommentService.updateComment(mockCommentLean._id.toString(), userId, updatedContent);
    ...
    it('should throw error if comment not found or user is not authorized', async () => {
        ...
        await expect(CommentService.updateComment('invalidId', userId, 'content'))
            .rejects.toThrow('Comment not found or you do not have permission to edit.');
    });
  });
  ...
  describe('deleteComment', () => {
    it('should delete a comment and update counts', async () => {
        ...
        await CommentService.deleteComment(commentToDelete._id.toString(), userId);
        
        expect(mockSession.startTransaction).toHaveBeenCalled();
        expect(mockComment.findOne).toHaveBeenCalledWith({ _id: commentToDelete._id.toString(), user: userId }, null, { session: mockSession });
  ...
  describe('toggleLike', () => {
    it('should add a like if not already liked', async () => {
        ...
        vi.spyOn(CommentService, 'getCommentById').mockResolvedValue(mockCommentLean as any);

        const result = await CommentService.toggleLike(mockCommentLean._id.toString(), userId);

        expect(mockLike.findOne).toHaveBeenCalled();
        expect(mockLike.create).toHaveBeenCalled();
        expect(mockComment.findByIdAndUpdate).toHaveBeenCalledWith(mockCommentLean._id.toString(), { $inc: { likesCount: 1 } }, { session: mockSession });
        expect(mockSession.commitTransaction).toHaveBeenCalled();
    ...
    it('should remove a like if already liked', async () => {
        ...
        expect(existingLike.deleteOne).toHaveBeenCalledWith({ session: mockSession });
        expect(mockComment.findByIdAndUpdate).toHaveBeenCalledWith(mockCommentLean._id.toString(), { $inc: { likesCount: -1 } }, { session: mockSession });
        expect(mockSession.commitTransaction).toHaveBeenCalled();
    });
  });
