...
describe('Bookmark Service', () => {
  ...
  describe('getBookmarksForUser', () => {
    ...
    it('should retrieve all bookmarks for a user with populated data', async () => {
      ...
      const planBookmark = result.bookmarks.find(
        (b) => b.onModel === 'TravelPlan',
      );
    ...
    it('should correctly handle pagination', async () => {
      ...
      await Bookmark.create({
        user: testUser._id,
        targetId: anotherPost._id,
        onModel: 'Post',
      });

      const page1 = await BookmarkService.getBookmarksForUser(
        testUser._id.toString(),
        'all',
        1,
        2,
      ...
      const page2 = await BookmarkService.getBookmarksForUser(
        testUser._id.toString(),
        'all',
        2,
        2,
  ...
  describe('getBookmarkIdsForUser', () => {
    it('should return a flat array of targetId strings', async () => {
      ...
      const result = await BookmarkService.getBookmarkIdsForUser(
        testUser._id.toString(),
      );
