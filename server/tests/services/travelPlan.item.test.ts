...
describe('TravelPlanService - Plan Item Management', () => {
        ...
        it('should add an item to a specific day successfully', async () => {
            ...
            const mockPlan = {
                ...
                schedule: [{
                    ...
                    items: [] as IPlanItem[], // SỬA LỖI: Chỉ định rõ kiểu cho mảng rỗng
                }],
                save: jest.fn().mockResolvedValue(true), // Mock hàm save
            };
            ...
            expect(mockPlan.save).toHaveBeenCalled(); // Kiểm tra xem plan đã được lưu lại chưa
            expect(mockPlan.schedule[0].items.length).toBe(1); // Kiểm tra item đã được thêm vào mảng chưa
    ...
    describe('updatePlanItem', () => {
        it('should update an item successfully', async () => {
            // Arrange
            const updateData = { title: 'Updated Title', cost: 100000 };
            const mockItem = { _id: mockItemId, title: 'Original Title', cost: 0 };
            const mockPlan = {
                author: mockUserId,
                schedule: [{
                    dayNumber: 1,
