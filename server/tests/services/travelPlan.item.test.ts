...
describe('TravelPlanService - Plan Item Management', () => {
  ...
  describe('addPlanItem', () => {
    it('should add an item to a specific day successfully', async () => {
      ...
      await TravelPlanService.addPlanItem(
        mockRequest.params.id,
        parseInt(mockRequest.params.dayNumber, 10),
        mockRequest.body,
        mockRequest.user as string,
      );
      ...
      expect(mockPlan.schedule[0].items[0].location).toEqual(
  ...
  describe('getPlanItem', () => {
    it('should retrieve a specific item successfully', async () => {
      ...
      // Act
      const result = await TravelPlanService.getPlanItem(
        mockPlanId,
        mockItemId.toHexString(),
        mockUserId,
      );
  ...
  describe('updatePlanItem', () => {
    it('should update an item successfully', async () => {
      ...
      // Act
      const result = await TravelPlanService.updatePlanItem(
        mockPlanId,
        mockItemId.toHexString(),
        updateData,
        mockUserId,
  ...
  describe('deletePlanItem', () => {
    it('should delete an item successfully', async () => {
      ...
      // Act
      const result = await TravelPlanService.deletePlanItem(
        mockPlanId,
        mockItemId.toHexString(),
        mockUserId,
      );
