...
describe('Comment Routes', () => {
    ...
    describe('GET /api/plans/:id/comments', () => {
        it('should return comments for a travel plan with status 200', async () => {
            ...
            mockCommentService.getCommentsForTarget.mockResolvedValue(serviceResponse as any);

            const response = await request(app)
                .get(`/api/plans/${planId}/comments?page=1&limit=10`);

            expect(response.status).toBe(200);
            expect(response.body).toEqual(serviceResponse);
            expect(mockCommentService.getCommentsForTarget).toHaveBeenCalledWith(planId, 'TravelPlan', 1, 10);
        });
    });
    ...
    describe('PATCH /api/comments/:commentId', () => {
        it('should update a comment and return it with status 200', async () => {
            ...
            expect(mockCommentService.updateComment).toHaveBeenCalledWith(commentId, mockUserId, 'Updated content');
        });
    });
    ...
    describe('DELETE /api/comments/:commentId', () => {
        it('should delete a comment and return status 200', async () => {
            ...
            expect(mockCommentService.deleteComment).toHaveBeenCalledWith(commentId, mockUserId);
        });
    });
    ...
    describe('POST /api/comments/:commentId/like', () => {
        it('should toggle a like and return the updated comment with status 200', async () => {
            ...
            expect(mockCommentService.toggleLike).toHaveBeenCalledWith(commentId, mockUserId);
        });

        it('should return 400 for an invalid commentId', async () => {
            const response = await request(app)
                .post('/api/comments/invalid-id/like');
     ...
     describe('GET /api/comments/:commentId/replies', () => {
        it('should return replies for a comment with status 200', async () => {
            const replies = [{...mockComment, _id: new Types.ObjectId().toString(), parentId: commentId }];
            ...
            expect(mockCommentService.getRepliesForComment).toHaveBeenCalledWith(commentId);
        });
    });
});