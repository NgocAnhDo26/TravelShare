...
import { MoreVertical, ThumbsUp, EyeOff, Pencil, Trash2, Loader2 } from 'lucide-react';
...
const renderContentWithMentions = (text: string | undefined, mentions: IMention[] | undefined) => {
    if (!text) return null;
        ...
        <>
            {parts.map((part, index) => {
                if (index % 2 === 1 && mentionMap.has(part)) {
                    ...
                    return (
                        <Link to={`/profile/${userId}`} key={index} className="text-blue-600 font-semibold no-underline hover:underline">
                            @{part}
                        </Link>
                    );
                }
            ...
            <div className='flex-1 group'>
                    ...
                    <div className="flex items-center gap-2">
                        <div className='w-fit max-w-full'>
                            <div className='bg-gray-100 dark:bg-gray-800 rounded-xl px-3 py-2 text-left min-w-[70px]'>
                                <Link to={`/profile/${userId}`} className="text-sm font-medium hover:underline">{name}</Link>
                                {comment.content && (
                                ...
                                {comment.imageUrl && (
                                    <Dialog>
                                        ...
                                        <DialogContent className="p-0 max-w-4xl w-auto bg-transparent border-none">
                                            <img src={comment.imageUrl} alt="comment-attachment" className="max-h-[90vh] w-auto rounded-lg object-contain" />
                                        </DialogContent>
                                    </Dialog>
                                )}
                            </div>
                        ...
                        <DropdownMenu>
                            <DropdownMenuTrigger asChild>
                                <Button variant='ghost' size='icon' className='opacity-0 group-hover:opacity-100 transition-opacity h-8 w-8 rounded-full'>
                ...
                <div className='flex items-center gap-4 text-xs mt-1 ml-3'>
                    ...
                    <button
                        ...
                        className={cn("font-semibold hover:underline", isLiked ? 'text-blue-600' : 'text-gray-500')}
                    >
                        Like
                    </button>
                    <button onClick={onSetReplyTarget} className="font-semibold text-gray-500 hover:underline">Reply</button>
                    {optimisticLikesCount > 0 && (
                        <LikesModal targetId={comment._id} onModel="Comment">
                            <div className="flex items-center gap-1 cursor-pointer">
                                ...
                                <span className="font-semibold text-gray-500">{optimisticLikesCount}</span>
                            </div>
                        </LikesModal>
                    )}
                </div>
