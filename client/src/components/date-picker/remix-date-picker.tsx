...
// Define props for our new component
interface InitialDateRangePickerProps
  extends React.HTMLAttributes<HTMLDivElement> {
  id?: string;
  onDateChange?: (date: DateRange | undefined) => void;
  initialDateRange?: DateRange; // Prop to set the initial date
}
...
export default function InitialDateRangePicker({
  ...
  // Set the component's internal state using the `initialDateRange` prop.
  const [date, setDate] = React.useState<DateRange | undefined>(
    initialDateRange,
  );
  const [isOpen, setIsOpen] = React.useState(false);

  // This effect ensures the component's date updates if the prop changes
  // (e.g., when the dialog is reopened with a new default date).
  React.useEffect(() => {
    ...
    <div className={cn('relative grid gap-2', className)}>
      <Popover open={isOpen} onOpenChange={setIsOpen}>
        <PopoverTrigger asChild>
          ...
          >
            {/* Label is hardcoded to "Dates" for the remix form's context */}
            <span
              className={cn(
