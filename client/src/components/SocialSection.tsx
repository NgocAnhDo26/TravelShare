...
const SocialSection: React.FC<SocialSectionProps> = ({
    ...
    const fetchUserLikes = useCallback(async (commentIds: string[]) => {
        ...
        } catch (error) { console.error("Failed to fetch user's likes", error); }
    }, [currentUser]);
    ...
    const fetchComments = useCallback(async (page = 1, loadMore = false) => {
        ...
        try {
            const response = await API.get<{
                ...
                currentPage: number,
                totalPages: number
            }>(`${targetApiPrefix}/${targetId}/comments`, { params: { page, sort: 'desc' } });
            ...
            if (currentUser) {
                const allCommentIds = response.data.comments.flatMap(c => [c._id, ...(c.replies?.map(r => r._id) || [])]);
    ...
    const handleDeleteComment = async (commentId: string, parentId: string | null): Promise<boolean> => {
        const confirmed = await new Promise<boolean>(resolve => toast(t => (
            <div className="flex flex-col items-center gap-4 p-2">
                <span className="text-center font-medium">Are you sure you want to delete this comment?</span>
                <div className="flex gap-2">
                    <Button variant="destructive" size="sm" onClick={() => { toast.dismiss(t.id); resolve(true); }}>Delete</Button>
                    <Button variant="outline" size="sm" onClick={() => { toast.dismiss(t.id); resolve(false); }}>Cancel</Button>
                </div>
            </div>
        ), { duration: Infinity }));
