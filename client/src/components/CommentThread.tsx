      ...
      {imagePreview && (
        <div className='relative w-fit self-start ml-12 mt-2'>
          ...
          <button
            type='button'
            onClick={removeImage}
            className='absolute top-1 right-1 bg-gray-900/50 text-white rounded-full p-1 hover:bg-gray-900/80'
          >
            <X className='h-4 w-4' />
          </button>
        </div>
...
const CommentThread: React.FC<ThreadProps> = ({ comment, ...rest }) => {
  ...
  useLayoutEffect(() => {
    ...
    const allElements = container.querySelectorAll<HTMLElement>(
      '.reply-item-container, .reply-input-container',
    );
    ...
    if (lastElement.classList.contains('reply-input-container')) {
      ...
      const targetRect = lastElement.getBoundingClientRect();
      const targetCenterY =
        targetRect.top - lineContainerRect.top + targetRect.height / 2;
  ...
  useEffect(() => {
    ...
    fetchReplies,
    replies.length,
    localComment.replyCount,
  ]);

  const handleSetReply = (
    targetComment: IComment,
    parentOfTarget: string | null,
    isDirectReplyAction: boolean = false,
  ) => {
    setReplyTarget({
  ...
  const getInitialReplyContent = () => {
    if (
      replyTarget &&
      replyTarget.shouldTag &&
      replyTarget.comment.user._id !== currentUserId
    ) {
  ...
  const visibleReplies = replies.filter(
    (r) => !rest.hiddenCommentIds.has(r._id),
  );

  return (
    <div className='comment-thread-container'>
      {(areRepliesVisible || isReplying || visibleReplies.length > 0) && (
        <div ref={lineRef} className='thread-line-main' />
      )}
              ...
              {visibleReplies.map((reply) => (
                <div key={reply._id} className='reply-item-container'>
                  ...
                  <CommentItem
                    ...
                    onEdit={(newContent) =>
                      handleEditReply(reply._id, newContent)
                    }
                    onSetReplyTarget={() =>
