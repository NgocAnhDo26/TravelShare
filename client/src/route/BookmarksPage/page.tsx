        ...
        <div className='px-1 py-1 '>
          <Menu.Item>
            {({ active }) => (
              <button
                onClick={onRemove}
                className={`${active ? 'bg-blue-500 text-white' : 'text-gray-900 dark:text-gray-200'} group flex w-full items-center rounded-md px-2 py-2 text-sm`}
              >
                <svg
                  xmlns='http://www.w3.org/2000/svg'
    ...
    <div className='flex items-start gap-4 bg-white/80 dark:bg-gray-800/60 backdrop-blur-sm border border-slate-200/80 dark:border-slate-700/60 rounded-xl p-4 shadow-lg hover:shadow-xl transition-all duration-300'>
      ...
      <div className='flex-1 min-w-0 text-left'>
        <Link to={`/plans/${plan._id}`} className='block'>
          <h3 className='text-md font-bold text-gray-900 dark:text-white truncate hover:underline'>
            {plan.title}
          </h3>
        </Link>
...
const BookmarksPage = () => {
  ...
  const [activeFilter, setActiveFilter] = useState<'all' | 'plans' | 'posts'>(
    'all',
  );
      ...
      <div className='p-4 rounded-lg bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 shadow-sm sticky top-24'>
        ...
        <nav className='flex flex-col space-y-2'>
          <button
            ...
            className={`px-4 py-2 text-left rounded-md transition-colors duration-200 ${activeFilter === 'all' ? 'bg-blue-100 dark:bg-blue-900 text-blue-700 dark:text-blue-200 font-semibold' : 'hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-300'}`}
          >
            All
          </button>
          <button
            onClick={() => setActiveFilter('plans')}
            className={`px-4 py-2 text-left rounded-md transition-colors duration-200 ${activeFilter === 'plans' ? 'bg-blue-100 dark:bg-blue-900 text-blue-700 dark:text-blue-200 font-semibold' : 'hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-300'}`}
          >
            Plans
          </button>
          <button
            onClick={() => setActiveFilter('posts')}
            className={`px-4 py-2 text-left rounded-md transition-colors duration-200 ${activeFilter === 'posts' ? 'bg-blue-100 dark:bg-blue-900 text-blue-700 dark:text-blue-200 font-semibold' : 'hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-300'}`}
          >
            Posts
          </button>
        </nav>
          ...
          <main className='w-full md:w-3/4 lg:w-4/5 xl:w-5/6'>
            <div className='flex flex-col gap-4'>
              {showLoading ? (
                ...
                <div className='text-center py-12 bg-gray-100 dark:bg-gray-800/50 p-6 rounded-lg'>
                  <h3 className='text-xl font-semibold text-gray-800 dark:text-gray-200'>
                    Nothing here yet
                  </h3>
                  <p className='text-gray-500 dark:text-gray-400 mt-2'>
                    Start saving plans and posts to see them here.
                  </p>
                </div>
              )}
            </div>
