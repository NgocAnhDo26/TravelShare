...
import ItinerarySection from '@/components/ItinerarySection'; // Assuming ItinerarySection is responsive internally
import TripHeader from '@/components/TripHeader'; // Assuming TripHeader is responsive internally
import SocialSection from '@/components/SocialSection'; // Assuming SocialSection is responsive internally
...
const PlanEditorPage: React.FC<{ editMode?: boolean }> = ({
    ...
    useEffect(() => {
        ...
        const fetchTripData = async () => {
            try {
                ...
                navigate('/itinerary'); // Redirect to a safe page if plan not found or error
            } finally {
                setIsLoading(false);
            }
        };
    ...
    if (isLoading) {
        return (
            ...
            <div className='flex flex-col h-full mx-4 sm:mx-8 md:mx-16 lg:mx-auto lg:max-w-7xl my-4 sm:my-6 md:my-8'>
                <Card className='w-full overflow-hidden pt-0'>
                    {/* Cover image skeleton - Adjusted height for responsiveness */}
                    <Skeleton className='w-full h-48 sm:h-64 md:h-72' />

                    {/* Header skeleton (title, meta) - Adjusted padding */}
                    <div className='p-4 sm:p-6 space-y-4'>
                        <Skeleton className='h-8 w-2/3' />
                        <Skeleton className='h-4 w-1/3' />
                    </div>

                    {/* Itinerary skeleton - Adjusted padding */}
    ...
    if (!planData) {
        return (
            // Adjusted margins for responsiveness
            <div className='flex flex-col h-full justify-center mx-4 sm:mx-8 md:mx-16 lg:mx-auto lg:max-w-7xl my-4 sm:my-6 md:my-8'>
    ...
    return (
        // Main container for the page content. Adjusted margins for responsiveness.
        // On large screens, it will be centered with a max-width.
        <div className='flex flex-col h-full mx-4 sm:mx-8 md:mx-16 lg:mx-auto lg:max-w-7xl my-4 sm:my-6 md:my-8'>
            <Card className='w-full overflow-hidden pt-0'>
                {/* TripHeader component is used here */}
            ...
            {/* SocialSection is assumed to be responsive internally */}
